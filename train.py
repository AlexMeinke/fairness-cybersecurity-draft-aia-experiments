    ...
    with open("train.log", "w") as f:
        with redirect_stdout(f):
            ...
            (
                ...
                train_loader_out,
                test_loaders_out,
            ) = fac.get_loaders(args)

            evaluator_list = fac.get_evaluators(args)(
                eval_model, device, test_loader_in, test_loaders_out
            )
            ...
            test_args = dotdict(
                {
                    "dataset": args.dset_in_name,
                    "adversarial": True,
                    "guaranteed": True,
                    "measures": ["AUC", "AUPR", "FPR@95"],
                    "eps": args.eval.eps,
                }
            )
            if (
                type(args.gpu) != omegaconf.listconfig.ListConfig
            ):  # don't waste multi-gpu resources
                eval_device = torch.device("cuda")
                eval_pipeline.gen_eval(doc_id, device=eval_device)
