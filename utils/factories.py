...
def get_model(arch):
    if arch.arch_style.lower() == "cnn":
        model = provable_classifiers.CNN_IBP(
            dset_in_name=arch.dset_in_name,
            size=arch.arch_size,
            last_bias=arch.use_last_bias,
            num_classes=arch.num_classes,
            last_layer_neg=arch.last_layer_neg,
        )
        if arch.last_layer_neg and arch.use_last_bias:
            with torch.no_grad():
                model.layers[-1].bias.data += 3.0
        ...
        model = dn.get_densenet(dataset=arch.dset_in_name, num_classes=2)
    ...
    if arch.file_path is not None:
        path = (
            arch.file_path
            if arch.file_path[0] == "/"
            else paths_config.project_folder + arch.file_path
        )
        try:
            ...
            state_dict = torch.load(
                paths_config.project_folder + path[idx:], map_location="cpu"
            )
        try:
...
def get_loaders(args):
    ...
    augmentation_train_in = {
        "crop": args.augmentation.crop,
        "HFlip": args.augmentation.hflip,
        "autoaugment": args.augmentation.autoaugment,
    }

    train_loader_in = dataloading.datasets_dict[args.dset_in_name](
        train=True,
    ...
    if args.dset_out_name == "TINY":
        train_loader_out = dataloading.datasets_dict[args.dset_out_name](
            train=True,
            batch_size=args.train.batch_size,
            augmentation=augmentation_train_in,
            dataloader_kwargs=dataloader_kwargs_tiny,
            shuffle=True,
        ...
        train_loader_out = dataloading.datasets_dict[args.dset_out_name](
            ...
            augmentation=augmentation_train_in,
            dataloader_kwargs=dataloader_kwargs,
        )  # exclude_cifar=args.augmentation.train_exclude causes problems if args.dset_out_name is not TINY
...
def get_test_loaders(args):
    ...
    test_loader_in = dataloading.datasets_dict[args.dset_in_name](
        train=False,
    ...
    for key in val_dict:
        loader = dataloading.datasets_dict[key](
            train=False,
            batch_size=args.eval.batch_size,
            augmentation=val_dict[key],
            dataloader_kwargs=dataloader_kwargs,
        )
        ...
        subset = np.random.choice(
            orig_size, size=min(orig_size, target_size), replace=False
        )
...
def get_evaluators(args):
    def fun(model, device, test_loader_in, test_loaders_out):
        ...
        IDEvaluator = evaluators.IDEvaluator(
            model, device, test_loader_in, from_logits=from_logits
        )

        if args.architecture.arch_style.lower() in ["joint", "rn"]:
            evaluatorOOD = evaluators.OODEvaluator(
                model, device, test_loader_in, test_loaders_out
            )
        else:
            evaluatorOOD = evaluators.BinaryEvaluator(
                model, device, test_loader_in, test_loaders_out
            )
...
def get_trainer_factory(train_args):
    schedule = schedules.schedule_dict[train_args.schedule.lr_schedule_type](
        train_args.schedule.lr_schedule, train_args.schedule.lr_schedule_epochs
    )
    kappa_schedule = schedules.schedule_dict[train_args.schedule.kappa_schedule_type](
        train_args.schedule.kappa_schedule, train_args.schedule.kappa_schedule_epochs
    )
    eps_schedule = schedules.schedule_dict[train_args.schedule.eps_schedule_type](
        train_args.schedule.eps_schedule, train_args.schedule.eps_schedule_epochs
    )
    ...
    if train_args.train_type == "GOOD":
        ...
        def fun(
            ...
            made_trainer = better.TrainerGOODER(
                model,
                device,
                in_loader,
                out_loader,
        ...
        def fun(
            ...
            made_trainer = better.TrainerPlain(
                model,
                device,
                in_loader,
                out_loader,
    ...
    elif train_args.train_type.lower() == "adversarial":
        ...
        def fun(
            ...
            made_trainer = better.TrainerAdversarial(
                model,
                device,
                in_loader,
                out_loader,
