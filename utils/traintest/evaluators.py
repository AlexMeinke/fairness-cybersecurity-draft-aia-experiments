...
class IDEvaluator:
    ...
    def run(self, logged_scalars={}, logged_vectors={}, logged_img={}):
        ...
        self.model.eval()
        acc, conf_in = ev.get_accuracy(
            self.model, self.device, self.in_loader, self.from_logits
        )
        confusion_matrix, confidence_matrix = ev.get_confusion_matrix(
            self.model, self.device, self.in_loader, self.from_logits
        )
...
class OODEvaluator:
    def __init__(
        self, model, device, in_loader, out_loader_dict, eps=0.01, use_ebm=False
    ):
        self.model = model
        self.device = device
        self.in_loader = in_loader
    ...
    def run(self, logged_scalars={}, logged_vectors={}, logged_img={}):
        ...
        for out_dset in self.out_loader_dict:
            ...
            if hasattr(self.model, "ibp_forward"):
                try:
                    ub = ev.get_ub_log_conf(
                        self.model, self.device, loader, eps=self.eps, num_classes=10
                    )
                except:
                    ub = (
                        ev.get_output_ub(self.model, self.device, loader, eps=self.eps)
                        .squeeze()
                        .numpy()
                    )
            ...
            if self.use_ebm:
                ...
                logged_scalars["test_ebm/" + out_dset + "_mean_energy"] = (
                    -neg_energy_out
                ).mean()
                ...
                cauc = ev.auroc_conservative(
                    neg_energy_in.numpy(), neg_energy_out.numpy()
                )
                logged_scalars["test_ebm/" + out_dset + "_cAUC"] = cauc

                ub = ev.get_ub_neg_energy(
                    self.model, self.device, loader, eps=self.eps, num_classes=10
                )
