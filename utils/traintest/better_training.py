...
from utils.adversarial.apgd import APGDAttack
...
class Trainer:
    def __init__(
        ...
        schedule=None,
        kappa_schedule=None,
        in_loss=nn.CrossEntropyLoss(),
        out_loss=losses.DirectLoss(),
        verbose=100,
        weight_decay=5e-4,
        noise_refresh_interval_schedule=None,
        in_refresh_interval_schedule=None,
    ):
        self.model = model
        self.device = device
        ...
        if schedule is None:
            self.lr_schedule = create_piecewise_constant_schedule(
                [0.1, 0.01, 0.001, 0.0001], [50, 75, 90, 100]
            )
        else:
            self.lr_schedule = schedule

        self.kappa_schedule = (
            0.0 * self.lr_schedule if kappa_schedule is None else kappa_schedule
        )
        ...
        if self.folder is not None:
            if epoch == len(self.lr_schedule) - 1:
                ...
                torch.save(
                    ...
                    self.folder + "/checkpoints/" + str(epoch) + ".pt",
                )
        ...
        if self.writer is not None:
            ...
            for key in self.logged_img:
                try:
                    ...
                    print(
                        f"Wtf is going on with {key} of shape {self.logged_img[key].shape}?"
                    )
                    print(e)
            for key in self.logged_img_batch:
                try:
                    self.writer.add_image(
                        key, self.logged_img_batch[key], epoch, dataformats="NCHW"
                    )
                except Exception as e:
                    print(
                        f"Wtf is going on with {key} of shape {self.logged_img_batch[key].shape}?"
                    )
                    print(e)
            ...
            if (batch_idx % self.verbose == 0) and self.verbose > 0:
                print(
                    "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                        epoch,
                        batch_idx * len(data),
                        len(self.in_loader.dataset),
                        100.0 * batch_idx / len(self.in_loader),
                        loss.item(),
                    )
                )
...
class TrainerGOODER(Trainer):
    def __init__(
        ...
        non_last_layer_params = filter(
            lambda p: id(p) not in last_layer_ids, model.parameters()
        )

        param_groups = [
            {
                "params": non_last_layer_params,
                "lr": self.lr_schedule[0],
                "weight_decay": self.weight_decay,
            },
            {
        ...
        ####### IBP #######
        if epsilon is None:
            self.eps_schedule = 0.01 + 0.0 * self.lr_schedule
        elif not hasattr(epsilon, "__iter__"):
            self.eps_schedule = epsilon + 0.0 * self.lr_schedule
        else:
...
class TrainerAdversarial(Trainer):
    ...
    def inner_train(self, epoch, out_loader):
        ...
        for batch_idx, (data, target) in enumerate(self.in_loader):
            data, target = data.to(self.device), target.to(self.device)
            half_bs = int(data.shape[0] / 2)

            data_clean, target_clean = data[:half_bs], target[:half_bs]
            out_in = self.model(data_clean)

            data_to_attack, target_adv = data[half_bs:], target[half_bs:]
            x_adv = self.attack.perturb(data_to_attack, target_adv)[0]
            out_adv = self.model(x_adv)

            in_loss = self.in_loss(out_in, target_clean)
            adv_loss = self.in_loss(out_adv, target_adv)
            self.logger["in_loss"].append(in_loss)
            self.logger["adv_loss"].append(adv_loss)

            loss = (in_loss.sum() + adv_loss.sum()) / (2 * half_bs)

            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

            self.logger["correct"].append(out_in.max(1)[1] == target_clean)
