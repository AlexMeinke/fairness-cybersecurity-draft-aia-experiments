...
class BinaryMarginLoss(nn.Module):
    ...
    def forward(self, output):
        return torch.logaddexp(
            torch.tensor([1.0], device=output.device), self.margin - output
        )
...
class QGOODLoss(nn.Module):
    ...
    def forward(self, ub_log_conf):
        ...
        above_quantile_indices = ub_log_conf.topk(h, largest=True)[
            1
        ]  # above or exactly at quantile, i.e. 'not below'.
