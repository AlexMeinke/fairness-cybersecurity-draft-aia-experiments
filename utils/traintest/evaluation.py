...
def auroc_conservative(values_in, values_out):
    ...
    for i in range(len(values_in)):
        s += (
            values_out < values_in[i]
        ).sum()  # + 0.5*(values_out == values_in[i]).sum()
    s = s.astype(float)
...
def auroc(values_in, values_out):
    ...
    y_score = np.concatenate(
        [np.nan_to_num(values_in, nan=0.0), np.nan_to_num(values_out, nan=0.0)]
    )
...
def get_accuracy(model, device, loader, from_logits=True):
    ...
    num = 0
    reduction = (
        (lambda x: torch.log_softmax(x, dim=1)) if from_logits else (lambda x: x)
    )
    for x, y in loader:
...
def get_confusion_matrix(model, device, loader, from_logits=True):
    ...
    reduction = (
        (lambda x: torch.softmax(x, dim=1)) if from_logits else (lambda x: x.exp())
    )
    with torch.no_grad():
        for x, y in loader:
...
def get_ub_log_conf(model, device, loader, eps, num_classes=10):
    ...
    for x, _ in loader:
        ...
        _, _, ud_logit_out_batch = model.ibp_elision_forward(
            torch.clamp(x - eps, 0, 1), torch.clamp(x + eps, 0, 1), num_classes
        )
    ...
    ub_el_out_log_confidences = ub_log_confs_from_ud_logits(
        ud_logit_out, force_diag_0=False
    )
...
def get_ub_neg_energy(model, device, loader, eps, num_classes=10):
    ...
    for x, _ in loader:
        x = x.to(device)
        lb, ub = model.ibp_forward(
            torch.clamp(x - eps, 0, 1), torch.clamp(x + eps, 0, 1)
        )
...
def ub_log_confs_from_ud_logits(
    ud_logits, force_diag_0=False
):  # upper bound differences matrix
...
def conf_stats_from_log_confs(log_confs, th, k):
    ...
    return (
        ...
        confidence_mean,
        confidence_median,
        confidences_below_th,
        confidences_above_th,
        lowest_conf_indices,
        highest_conf_indices,
    )
