...
def get_conf_lb(model, device, loader, args):
    ...
    if num_classes == 1:
        ...
        reduction = (
            (lambda x: torch.softmax(x, dim=1).max(1)[0])
            if from_logits
            else (lambda x: x.exp().max(1)[0])
        )
