...
class RandClassLoss(nn.Module):
    def __init__(self, y, y_target=None, classes=10, reduction="mean"):
        ...
        if y_target is None:
            y_target = torch.tensor(
                [random.choice([i for i in range(classes) if i != l]) for l in y]
            )
    ...
    def forward(self, x, y=None):
        if y is not None:
            ...
            y_target = torch.tensor(
                [random.choice([i for i in range(self.classes) if i != l]) for l in y],
                device=x.device,
            )

            index_mask = (torch.cumsum(torch.ones(y.shape[0], self.classes), 1) - 1).to(
                x.device
            )
