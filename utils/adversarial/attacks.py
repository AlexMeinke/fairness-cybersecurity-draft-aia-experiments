...
class Contraster(AdversarialNoiseGenerator):
    ...
    def forward(self, x):
        eps = self.eps
        s = (x > (1 - eps)).float() + torch.clamp(
            x * (x <= (1 - eps)).float() - eps, 0, 1
        )
        return s - x
    ...
    def forward(self, x):
        if self.lambda_scheme == "normal":
            ...
            while sum(reject_idcs > 0):
                lambda_targets[reject_idcs] = (
                    math.sqrt(self.rho)
                    * torch.randn(sum(reject_idcs), device=x.device).abs()
                    + 1e-8
                )
                reject_idcs = lambda_targets > 1
        ...
        for i in range(self.iterations + 1):
            ...
            with torch.no_grad():
                ...
                if self.low_rank:
                    ...
                    grad = torch.bmm(
                        self.S[y, :], grad.view(x.shape[0], -1, 1)
                    ).view_as(grad)
