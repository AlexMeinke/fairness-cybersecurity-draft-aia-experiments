...
def getloader_MNIST(train, batch_size, augmentation, dataloader_kwargs):
    if list(augmentation) == []:
        ...
        transform = transforms.Compose(
            [transforms.RandomCrop(28, augmentation["crop"]), transforms.ToTensor(),]
        )
    else:
        raise KeyError(
            f"Only crop augmentation supported for MNIST. Got {augmentation}."
        )
    dset = datasets.MNIST(
        paths_config.location_dict["MNIST"], train, download=True, transform=transform
    )
    loader = torch.utils.data.DataLoader(
        dset, batch_size=batch_size, shuffle=train, **dataloader_kwargs
    )
    return loader
...
def getloader_FashionMNIST(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.FashionMNIST(
        paths_config.location_dict["FashionMNIST"],
        train,
        download=True,
        transform=transform,
    )
...
def getloader_EMNIST_Letters(train, batch_size, augmentation, dataloader_kwargs):
    if list(augmentation) == []:
        transform = transforms.Compose(
            [lambda x: np.array(x).T, transforms.ToTensor(),]
        )
    elif list(augmentation) == ["crop"]:
        transform = transforms.Compose(
            [
    ...
    dset = datasets.EMNIST(
        paths_config.location_dict["EMNIST"],
        split="letters",
        train=train,
        download=True,
        transform=transform,
...
def getloader_Omniglot(train, batch_size, augmentation, dataloader_kwargs):
    if list(augmentation) == []:
        transform = transforms.Compose(
            [transforms.Resize(size=(28, 28)), transforms.ToTensor(), lambda x: 1 - x,]
        )
    elif list(augmentation) == ["crop"]:
        transform = transforms.Compose(
            [
    ...
    dset = datasets.omniglot.Omniglot(
        paths_config.location_dict["Omniglot"],
        download=True,
        background=train,
        transform=transform,
    )
...
def getloader_NotMNIST(train, batch_size, augmentation, dataloader_kwargs):
    if list(augmentation) == []:
        transform = transforms.Compose(
            [transforms.ToTensor(), lambda x: x[0].view(1, 28, 28),]
        )
    elif list(augmentation) == ["crop"]:
    ...
    if train:
        raise ValueError(
            f"ImageNet- only existes for the validation data of ImgeNet exists. train is set to {train}, which is not allowed."
        )

    dset = datasets.ImageFolder(
        "../datasets/notMNIST_small", transform=transform
    )  # deleted corrupted files ['A'] = {'RGVtb2NyYXRpY2FCb2xkT2xkc3R5bGUgQm9sZC50dGY=.png'}, ['F'] = {'Q3Jvc3NvdmVyIEJvbGRPYmxpcXVlLnR0Zg==.png'}
    loader = torch.utils.data.DataLoader(
        dset, batch_size=batch_size, shuffle=train, **dataloader_kwargs
    )
    return loader
...
def getloader_TINY(
    ...
    if augmentation.get("28g"):  # MNIST style format
        ...
        if not augmentation.keys() <= {"28g", "crop", "HFlip", "autoaugment"}:
            raise ValueError(
                f"At least one of the augmentations {augmentation.keys()} is not supported."
            )
        transform = transforms.Compose(
            []
            + augmentation.get("HFlip", False) * [transforms.RandomHorizontalFlip(),]
            + bool(augmentation.get("crop", 0))
            * [transforms.RandomCrop(32, augmentation.get("crop", 0)),]
    ...
    if train:
        ...
        dset = torch.utils.data.Subset(
            tiny_dset, np.random.choice(100000, size=1000, replace=False)
        )
        dset.__repr__ = tiny_dset.__repr__
    loader = torch.utils.data.DataLoader(
        dset,
...
def getloader_CIFAR10(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.CIFAR10(
        paths_config.location_dict["CIFAR10"], train, download=True, transform=transform
    )
    loader = torch.utils.data.DataLoader(
        dset, batch_size=batch_size, shuffle=train, **dataloader_kwargs
    )
...
def getloader_CIFAR100(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.CIFAR100(
        paths_config.location_dict["CIFAR100"],
        train,
        download=True,
        transform=transform,
    )
...
def getloader_SVHN(train, batch_size, augmentation, dataloader_kwargs):
    ...
    if train:
        ...
        split = "test"
    dset = datasets.SVHN(
        paths_config.location_dict["SVHN"],
        split=split,
        download=True,
        transform=transform,
    )
...
def getloader_Noise(train, batch_size, augmentation, dataloader_kwargs, noise_fn):
    ...
    if augmentation.get("28g"):
        ground_ds = datasets.MNIST(
            paths_config.location_dict["MNIST"],
            train=train,
            download=True,
            transform=transform,
        )
        ...
        ground_ds = datasets.CIFAR10(
            paths_config.location_dict["CIFAR10"],
            train=train,
            download=True,
            transform=transform,
        )
...
def getloader_Uniform(train, batch_size, augmentation, dataloader_kwargs):
    return getloader_Noise(
        train, batch_size, augmentation, dataloader_kwargs, noisefunctions.noise_uniform
    )
...
def getloader_Smooth(train, batch_size, augmentation, dataloader_kwargs):
    return getloader_Noise(
        ...
        augmentation,
        dataloader_kwargs,
        noisefunctions.noise_low_freq,
    )


def getloader_Black(train, batch_size, augmentation, dataloader_kwargs):
    return getloader_Noise(
        train, batch_size, augmentation, dataloader_kwargs, noisefunctions.monochrome(0)
    )
...
def getloader_LSUN_CR(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.LSUN(
        paths_config.location_dict["LSUN"], classes=classes, transform=transform
    )
    loader = torch.utils.data.DataLoader(
        dset, batch_size=batch_size, shuffle=train, **dataloader_kwargs
    )
...
def getloader_ImageNetM(train, batch_size, augmentation, dataloader_kwargs):
    ...
    dset = datasets.ImageFolder(
        paths_config.location_dict["ImageNet-"], transform=transform
    )
...
def getloader_RImageNet(train, batch_size, augmentation, dataloader_kwargs):
    if (
        len(augmentation) > 0
        and "autoaugment" in augmentation
        and augmentation["autoaugment"]
    ):
        ...
        augm_type = "none"
    return imgnet.get_restrictedImageNet(
        train=train,
        batch_size=batch_size,
        shuffle=train,
        augm_type=augm_type,
...
def getloader_NotRImageNet(train, batch_size, augmentation, dataloader_kwargs):
    ...
    if "sampler" in dataloader_kwargs:
        ...
        sampler = None
    return imgnet.get_restrictedImageNetOD(
        train=train,
        batch_size=batch_size,
        shuffle=train,
        augm_type=augm_type,
...
def getloader_celebA(train, batch_size, augmentation, dataloader_kwargs):
    ...
    if train:
        transform = transforms.Compose(
            ...
            + augmentation.get("HFlip", False) * [transforms.RandomHorizontalFlip(),]
            + bool(augmentation.get("crop", 0))
            * [transforms.RandomCrop(width, augmentation.get("crop", 0)),]
            + augmentation.get("autoaugment", False) * [AutoAugment(),]
            + [transforms.ToTensor(),]
        )
    else:
    ...
    # IT USED TO WORK DETERMINISTICALLY
    # rng = np.random.default_rng(1306)
    # weight = rng.standard_normal(40)
    # # weight[20] = 2.5
    # bias = -5
    # label_reduction = lambda y: np.int64((y @ weight + bias) > 0)
    ...
    prob_computation = lambda y: sigmoid(y @ weight + bias)
    label_reduction = lambda y: np.int64(
        sigmoid(y @ weight + bias) > rng.random(len(y))
    )
    ...
    dset = CelebaDataset(
        ...
        prob_computation=prob_computation,
    )
    loader = torch.utils.data.DataLoader(
        dset, batch_size=batch_size, shuffle=train, **dataloader_kwargs
    )
...
class CelebaDataset(torch.utils.data.dataset.Dataset):
    """Custom Dataset for loading CelebA face images"""

    def __init__(
        self, train, transform=None, label_reduction=None, prob_computation=None
    ):
        self.prob_computation = prob_computation
        self.train = train
        self.classes = ["Reject", "Accept"]

        # produce a randomly chosen but fixed test set
        rng = np.random.default_rng(1306)
        subset = rng.choice(202599, size=10000, replace=False, shuffle=False)
        if train:
            subset = np.setdiff1d(np.arange(202599), subset)
        self.subset = subset

        path = paths_config.location_dict["CelebA"]
        txt_path = os.path.join(path, "list_attr_celeba.txt")
        img_dir = os.path.join(path, "img_align_celeba")
        df = pd.read_csv(txt_path, sep="\s+", skiprows=1)

        self.df = df.iloc[subset]
        self.img_dir = img_dir
        self.txt_path = txt_path

        self.img_names = df.index.values[subset]
        self.y = df.values[subset]
        if label_reduction:
            self.y = label_reduction(self.y)
        self.transform = transform
...
def getloader_tiny_open_images(
    train,
    batch_size,
    augmentation,
    dataloader_kwargs,
...
def get_val_out_loaders(dset_in_name, batch_size, dataloader_kwargs):
    return [
        ...
        for name, augm in val_loader_out_dicts[dset_in_name].items()
    ]
...
def get_test_out_loaders(dset_in_name, batch_size, dataloader_kwargs):
    return {
        ...
        for name, augm in test_loader_out_dicts[dset_in_name].items()
    }
