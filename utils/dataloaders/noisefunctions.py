...
def noise_permute(datapoint):
    ...
    imgn_flat_permuted = np.random.permutation(
        imgn_flat
    )  # this function shuffles the first axis
    imgn_permuted = imgn_flat_permuted.reshape(imgn.shape)
    return (
        torch.from_numpy(np.transpose(imgn_permuted, (2, 0, 1))),
        label,
    )  # (label, 'permuted')
...
def filter_gauss(datapoint, srange=[1, 1]):
    ...
    return (
        torch.from_numpy(np.transpose(imgn_gaussed, (2, 0, 1))),
        label,
    )  # + ('gauss', sigma)
...
def noise_uniform(datapoint):
    ...
    if imgn.shape[2] != 1:
        ...
        imgn_random = np.float32(np.random.uniform(size=imgn.shape))
        assert (
            torch.from_numpy(np.transpose(imgn_random, (2, 0, 1))).shape == img.shape
        ), (
            "torch.from_numpy(np.transpose(imgn_random, (2,0,1))).shape wrong: "
            + str(torch.from_numpy(np.transpose(imgn_random, (2, 0, 1))).shape)
        )
        return torch.from_numpy(np.transpose(imgn_random, (2, 0, 1))), label
...
class rectangles:
    ...
    def draw(self, datapoint):
        ...
        img[
            :,
            h * h_section_length : (h + 1) * h_section_length,
            v * v_section_length : (v + 1) * v_section_length,
        ] = 1
        return img, label
